...
class Spider(Spider):
    def init(self, extend="{}"):
        self.host = "https://5e543.vgrlialp.cc/"
        self.headers = {
            'Referer': self.host,
    ...
    def homeContent(self, filter):
        ...
        for a in rsp('#app-nav a.app-nav-item').items():
            ...
            if href and '/videos/' in href:
                type_name = a.text().strip().encode('latin-1').decode('utf-8')
                
                # This condition checks for and skips the "精选G片" category
                if type_name == "精选G片":
                    continue
        ...
        for a in rsp('.dx-line-tab-item a').items():
            ...
            if href != 'play':
                filter.append({
                    'n': a.text().strip().encode('latin-1').decode('utf-8'),
                    'v': href
                })
        return {
            'class': CLASSES,
